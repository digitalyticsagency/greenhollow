...
const SND = (function(){
  ...
  let wind=null, rain=null, rainF=null, birdTimer=0, musTimer=0, musStep=0;
  ...
  function scheduleBird(){
    ...
    birdTimer = setTimeout(()=>{
      if(!muted && ambG && ambG.gain.value>0.01){
  ...
  function startMusic(){
    ...
    const step = ()=>{
      const c=ctx();
      if(c && !muted && musG.gain.value>0.005){
  ...
  function applyMix(){
    const c=ctx(); if(!c) return;
    master.gain.setTargetAtTime(muted?0:vol.master, c.currentTime, 0.2);
    ambG.gain.setTargetAtTime(muted?0:(S&&S.snd&&S.snd.amb===false?0:vol.amb), c.currentTime, 0.8);
    musG.gain.setTargetAtTime(muted?0:(S&&S.snd&&S.snd.mus===false?0:vol.mus), c.currentTime, 1.2);
