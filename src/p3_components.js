...
function plant(cr,x,y,st,seed,pest){
  ...
  const R = 3.1*sc;
  /* Crops used to sway. Wind now moves trees only, so this is a plain
     group - kept as a group because the leaves are positioned relative
     to it and removing it would move every plant. */
  s += '<g>';
  for(let i=0;i<4;i++){
...
ART.cabin = (w,h)=>{
  ...
  /* the main mass sits behind and above the verandah - drop a soft band
...
ART.battery = (w,h)=>{
  ...
  s += edge(w*0.14, h*0.16, w*0.72, h*0.66, 2);   /* the cabinet, not the tile */
  return s;
};
...
ART.well = (w,h)=>{
  ...
  s += `<ellipse cx="${n(w/2+1.5)}" cy="${n(h/2+2)}" rx="${n(Math.min(w,h)/2-2)}" ry="${n(Math.min(w,h)/2-3.5)}" fill="url(#gShadow)" opacity=".55"/>`;
  s += `<circle cx="${n(w/2)}" cy="${n(h/2)}" r="${n(Math.min(w,h)/2-3)}" fill="url(#gStone)"/>`;
  /* the shaft is a hole, so its near rim catches light and its far rim
     does not - one arc is the whole trick */
  s += `<path d="M${n(w/2-(Math.min(w,h)/2-4.2))} ${n(h/2)} a ${n(Math.min(w,h)/2-4.2)} ${n(Math.min(w,h)/2-4.2)} 0 0 1 ${n((Math.min(w,h)/2-4.2)*2)} 0" fill="none" stroke="var(--a-shadow)" stroke-width="1.6" opacity=".4"/>`;
...
ART.fodder = (w,h)=>{
  ...
  for(let i=0;i<Math.floor(w*h/26);i++){
    ...
    /* two tones of stalk on the same hash - the darker ones read as the
       row behind, which is most of what makes a crop look deep */
    s += `<path d="M${n(x)} ${n(h-4)} q ${n((hash(i)-0.5)*3)} -6 ${n((hash(i+2)-0.5)*4)} -9" stroke="${hash(i*3.1)>0.5?'#c9c26a':'#a89c52'}" stroke-width="1.1" fill="none"/>`;
  }
  s += `</g>`;
  return s;
};
...
ART.packing = (w,h,ob)=>{
  ...
  s += `<ellipse cx="${n(w*0.23)}" cy="${n(h*0.72)}" rx="${n(w*0.15)}" ry="2.4" fill="url(#gShadow)" opacity=".5"/>`;
  s += `<rect x="${n(w*0.1)}" y="${n(h*0.68)}" width="${n(w*0.24)}" height="5" rx="1" fill="#a1743f"/>`;
  s += `<rect x="${n(w*0.1)}" y="${n(h*0.68)}" width="${n(w*0.24)}" height="1.6" rx="0.8" fill="#c08f52"/>`;
  return s;
};
...
ART.tea_kiosk = (w,h)=>{
  ...
  s += `<ellipse cx="${n(w*0.5+0.8)}" cy="${n(h*0.62+2.6)}" rx="3.4" ry="1.4" fill="url(#gShadow)" opacity=".5"/>`;
  s += `<circle cx="${n(w*0.5)}" cy="${n(h*0.62)}" r="3" fill="url(#gPlaster)"/>`;
  s += `<circle cx="${n(w*0.5-0.9)}" cy="${n(h*0.62-0.9)}" r="1.2" fill="#fff" opacity=".55"/>`;
  return s;
};
ART.gift_shop = (w,h)=>{
  let s = building(w,h,{roof:'url(#gRoof)'});
  /* A 5px-tall shopfront can never clear edge()'s size gate, so it gets
     the frame drawn explicitly rather than a call that returns ''. */
  s += `<rect x="${n(w*0.28)}" y="${n(h*0.62)}" width="${n(w*0.42)}" height="5" rx="1" fill="url(#gGlass)"/>`;
  s += `<rect x="${n(w*0.28)}" y="${n(h*0.62)}" width="${n(w*0.42)}" height="5" rx="1" fill="none" stroke="var(--a-edge)" stroke-width="0.6"/>`;
  s += `<rect x="${n(w*0.28)}" y="${n(h*0.62)}" width="${n(w*0.42)}" height="1.6" rx="0.8" fill="#fff" opacity=".3"/>`;
  return s;
};
ART.glamping = (w,h)=>{
  const cx=w/2;
...
ART.deck = (w,h)=>{
  ...
  s += edge(0.5, 0.5, w-1, h-2, 2.5);
  return s;
};
ART.playground = (w,h)=>{
  let s = patch(w,h,'#93bd64',41,1);
  s += `<ellipse cx="${n(w*0.24)}" cy="${n(h*0.62)}" rx="${n(w*0.18)}" ry="${n(h*0.2)}" fill="url(#gEarthDry)"/>`;
  ...
  /* the A-frame and the slide were floating - one small pool of shade at
     each foot puts them back on the grass */
  s += `<ellipse cx="${n(w*0.56)}" cy="${n(h*0.71)}" rx="${n(w*0.12)}" ry="2.6" fill="url(#gShadow)" opacity=".5"/>`;
  ...
  s += `<ellipse cx="${n(w*0.89)}" cy="${n(h*0.64)}" rx="${n(w*0.05)}" ry="2.2" fill="url(#gShadow)" opacity=".5"/>`;
...
ART.firepit = (w,h)=>{
  ...
  for(let i=0;i<9;i++){ const a=(i/9)*Math.PI*2;
    s += `<ellipse cx="${n(cx+Math.cos(a)*R*0.82+0.7)}" cy="${n(cy+Math.sin(a)*R*0.82+1.1)}" rx="2.6" ry="1.5" fill="url(#gShadow)" opacity=".6"/>`;
...
ART.gate   = (w,h)=>{
  ...
  s += `<ellipse cx="${n(w*0.07+3.5)}" cy="${n(h*0.88)}" rx="4.2" ry="1.7" fill="url(#gShadow)" opacity=".6"/>`;
  s += `<ellipse cx="${n(w*0.86+3.5)}" cy="${n(h*0.88)}" rx="4.2" ry="1.7" fill="url(#gShadow)" opacity=".6"/>`;
  ...
  s += `<rect x="${n(w*0.07)}" y="${n(h*0.18)}" width="1.7" height="${n(h*0.7)}" rx="0.8" fill="#fff" opacity=".22"/>`;
  s += `<rect x="${n(w*0.86)}" y="${n(h*0.18)}" width="1.7" height="${n(h*0.7)}" rx="0.8" fill="#fff" opacity=".22"/>`;
...
ART.worker_cottage = (w,h,ob)=>{
  ...
  ['#e8eef0','#c8583f','#e8c25a'].forEach((c,i)=>{
    s += `<rect x="${n(w*(0.24+i*0.2))}" y="${n(h*0.78)}" width="5" height="7" rx="1" fill="${c}"
