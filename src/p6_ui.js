...
function renderStats(){
  ...
  const items = [
    ...
    {ic:`<svg class="ic" viewBox="0 0 16 16"><path d="M8 1.5S3 7 3 10a5 5 0 0010 0c0-3-5-8.5-5-8.5z" fill="#6fb6d8"/></svg>`,
      ...
      tip:`<b>Water</b>Beds drink this when you water them.<hr><div class="tl"><span>Storage</span><b>${Math.round(S.water)} / ${wcap} L</b></div><div class="tl"><span>Refill per day</span><b>+${num2(st.waterGain)} L</b></div>${st.waterCap<50?'<div class="warn">Build rain tanks to store more.</div>':''}`},
    {ic:`<svg class="ic" viewBox="0 0 16 16"><path d="M9 1L3 9h4l-1 6 6-8H8z" fill="#f0c14b"/></svg>`,
      v:st.power+'', s:`<small>/${num2(st.use)}kW</small>`, warn:st.short,
      tip:`<b>Power</b><div class="tl"><span>Generating</span><b>${st.power} kW</b></div><div class="tl"><span>Using</span><b>${num2(st.use)} kW</b></div><hr>${st.short?'<span class="warn">Short of power — greenhouses and workshops run at 40%.</span>':'<span class="tg">Comfortable. Weather changes solar output a lot.</span>'}`},
    {ic:`<svg class="ic" viewBox="0 0 16 16"><path d="M2 12c3-1 4-4 4-7 2 1 3 3 3 5 1-1 2-3 2-5 2 2 3 5 3 7z" fill="#c9c26a"/></svg>`,
      ...
      tip:`<b>Animal feed</b>Every animal eats every day.<hr><div class="tl"><span>Growing</span><b>+${num2(st.feedGain)}/day</b></div><div class="tl"><span>Eaten</span><b>${S.objs.reduce((a,o)=>a+(BPMAP[o.bp].feed||0)*(o.animals||0),0).toFixed(1)}/day</b></div>${S.feed<4?'<div class="warn">Nearly out — build a fodder patch or buy feed.</div>':''}`},
